...
import com.jalabot.jalabotapp.config.KafkaProperties;
...
class JalaBotKafkaResourceIT {
    ...
    void setup() {
        ...
        JalaBotKafkaResource kafkaResource = new JalaBotKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jala-bot-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jala-bot-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
